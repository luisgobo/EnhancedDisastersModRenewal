...
namespace NaturalDisastersRenewal.Models.Setup
{
    public class DisasterSetupModel
    {
        ...
        public void ApplyRealTimeFrequencyHarmony(RealTimeDisasterFrequencyPreset frequency)
        {
            RealTimeFrequencyHarmony = frequency;
            ForestFire.SetRealTimeForestFireFrequency(frequency);
            Thunderstorm.SetRealTimeThunderstormFrequency(frequency);
            Sinkhole.SetRealTimeSinkholeFrequency(frequency);
            Tornado.SetRealTimeTornadoFrequency(frequency);
            Tsunami.SetRealTimeTsunamiFrequency(frequency);
            Earthquake.SetRealTimeEarthquakeFrequency(frequency);
            MeteorStrike.SetRealTimeMeteorFrequency(frequency);
        }

        public bool IsRealTimeFrequencyMismatched(RealTimeDisasterFrequencyPreset disasterFrequency)
        {
            return DisasterSimulationUtils.IsRealTimeModActive() && disasterFrequency != RealTimeFrequencyHarmony;
        }
