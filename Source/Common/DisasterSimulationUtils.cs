...
namespace NaturalDisastersRenewal.Common
{
    public static class DisasterSimulationUtils
    {
        ...
        public static string[] GetAllEvacuationOptions(bool allowsFocusedEvacuation = false)
        {
            ...
            {
                string focusedEvacuation = LocalizationService.Get("evacuation.focused");
        ...
        public static string FormatTimeSpan(float daysFloat)
        {
            if (daysFloat <= 0)
            {
                return "0 " + LocalizationService.Get("time.day") + "s";
            }

            if (daysFloat < 1)
            {
                return LocalizationService.Get("time.less_than_one_day");
            }

            if (daysFloat < 60)
            {
                return FormatValue(daysFloat, LocalizationService.Get("time.day"));
            }
            ...
            if (months < 13)
            {
                ...
                return FormatValue(months, LocalizationService.Get("time.month")) + " " + LocalizationService.Get("time.and") + " " + FormatValue(days, LocalizationService.Get("time.day"));
            }
            ...
            return FormatValue(years, LocalizationService.Get("time.year")) + " " + LocalizationService.Get("time.and") + " " + FormatValue(months, LocalizationService.Get("time.month"));
        }
