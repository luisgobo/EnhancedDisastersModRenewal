...
namespace NaturalDisastersRenewal.UI.ComponentHelper
{
    public static class ToggleButtonIconHelper
    {
        ...
        private static UITextureAtlas activeAtlas;
        private static UITextureAtlas inactiveAtlas;
        ...
        public static bool Apply(UIButton button, bool isActive)
        {
            ...
            var iconWidth = Mathf.Max(1f, button.width - IconPadding * 2f);
            var iconHeight = Mathf.Max(1f, button.height - IconPadding * 2f);
            sprite.size = new Vector2(iconWidth, iconHeight);
            sprite.relativePosition = new Vector3(IconPadding, IconPadding);
        ...
        private static UITextureAtlas GetAtlas(bool active)
        {
            ...
            {
                if (activeAtlas == null)
                    activeAtlas = CreateAtlas(ActiveAtlasName, ActiveSpriteName, ActiveResourceName);

                return activeAtlas;
            }

            if (inactiveAtlas == null)
                inactiveAtlas = CreateAtlas(InactiveAtlasName, InactiveSpriteName, InactiveResourceName);

            return inactiveAtlas;
        }
